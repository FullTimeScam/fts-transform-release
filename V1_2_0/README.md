...
> 현재 버전: **v1.2.0** ([CHANGELOG](CHANGELOG.md))
...
## **v1.2.0 변경점**
— 작업 산출물(코드·문서·스크립트·에셋·리서치 결과물)이 작업 메타 폴더에 머무르거나 임의 위치에 흩어지지 않도록, 프로젝트 타입별 표준 폴더 매핑(`{{SRC_PATH}}`/`{{TESTS_PATH}}`/`{{SCRIPTS_PATH}}`/`{{DOCS_PATH}}`/`{{RESEARCH_OUTPUT_PATH}}` + derived `{{ARTIFACT_FOLDERS_TABLE}}`)을 추가했다. 도메인 룰 4종에 "산출물 배치" 섹션 + plan.md 의 각 step 에 "출력 경로" 필수 필드 + planner 에이전트 검증 항목이 들어갔다. 상세는 [CHANGELOG.md](CHANGELOG.md).
